...
edit_subgraph = mcp_server.edit_subgraph
get_node_info = mcp_server.get_node_info
summarize_workflow = mcp_server.summarize_workflow
...
class TestEditGraphOperations:
    ...
    def test_create_reports_root_locator(self, mock_comfyui):
        result = edit_graph([{"action": "create", "node_type": "KSampler"}])

        assert "created: root:1" in result
...
class TestEditSubgraph:

    def test_create_inherits_wrapper_graph_id(self, mock_comfyui):
        mock_comfyui["cmd"].return_value = {"node_id": "61", "graph_id": SUBGRAPH_ID, "size": [300, 100]}

        result = edit_subgraph(SUBGRAPH_ID, [{"action": "create", "node_type": "KSampler"}])
    ...
    def test_connect_and_set_use_scoped_local_ids(self, mock_comfyui):
        mock_comfyui["cmd"].return_value = {"status": "ok", "graph_id": SUBGRAPH_ID}

        result = edit_subgraph(SUBGRAPH_ID, [
            {"action": "connect", "from_node": "45", "from_slot": 0, "to_node": "11", "to_slot": 0},
            {"action": "set", "node_id": "11", "property": "cfg", "value": 1.0},
        ])

        assert "ok: 2/2" in result
        first_call = mock_comfyui["cmd"].call_args_list[0]
        assert first_call[0][1]["graph_id"] == SUBGRAPH_ID
        assert first_call[0][1]["from_node_id"] == "45"
        assert first_call[0][1]["to_node_id"] == "11"

        second_call = mock_comfyui["cmd"].call_args_list[1]
        assert second_call[0][1]["graph_id"] == SUBGRAPH_ID
        assert second_call[0][1]["node_id"] == "11"

    def test_ref_keeps_wrapper_scoped_locator(self, mock_comfyui):
        mock_comfyui["cmd"].side_effect = [
            {"node_id": "61", "graph_id": SUBGRAPH_ID, "size": [300, 100]},
            {"status": "ok", "graph_id": SUBGRAPH_ID},
        ]
    ...
    def test_conflicting_explicit_graph_id_is_rejected(self, mock_comfyui):
        result = edit_subgraph(SUBGRAPH_ID, [{"action": "create", "node_type": "KSampler", "graph_id": OTHER_SUBGRAPH_ID}])

        assert "error:" in result
        assert "graph_id does not match edit_subgraph graph_id" in result
        mock_comfyui["cmd"].assert_not_called()


class TestGetNodeInfoSubgraphs:

    def test_get_node_info_reports_root_locator_for_root_node(self):
        workflow = {
            "workflow": {
                "nodes": [
                    {
        ...
        with patch("mcp_server.get_workflow", return_value=workflow), \
             patch("mcp_server.get_object_info_cached", return_value={"KSampler": {}}):
            result = get_node_info("7")

        assert "node root:7: KSampler" in result
        assert "locator: root:7" in result
        assert "graph: root" in result
...
class TestWorkflowSummaries:

    def test_summarize_workflow_uses_canonical_locators_across_graphs(self):
        workflow = {
            "workflow": {
                "nodes": [
                    {"id": 60, "type": SUBGRAPH_ID, "title": "Subgraph Wrapper", "pos": [0, 0], "size": [200, 100]},
                    {"id": 7, "type": "KSampler", "title": "Root Sampler", "pos": [300, 0], "size": [240, 120]},
                ],
                "links": [[1, 60, 0, 7, 0, "MODEL"]],
                "definitions": {
                    "subgraphs": [
                        {
                            "id": SUBGRAPH_ID,
                            "nodes": [
                                {"id": 19, "type": "KSampler", "title": "Nested Sampler", "pos": [20, 40], "size": [220, 140]},
                                {"id": 45, "type": "CLIPTextEncode", "title": "Nested Prompt", "pos": [280, 40], "size": [200, 100]},
                            ],
                            "links": [[2, 45, 0, 19, 0, "CONDITIONING"]],
                        }
                    ]
                },
            }
        ...
        assert "nodes[4]{id,locator,graph,type,title,x,y,w,h}:" in result
        assert "60,root:60,root" in result
        assert "7,root:7,root" in result
        assert f"19,{SUBGRAPH_ID}:19,{SUBGRAPH_ID}" in result
        assert f"45,{SUBGRAPH_ID}:45,{SUBGRAPH_ID}" in result
        assert "connections[2]{graph,from_locator,from_slot,to_locator,to_slot,type}:" in result
        assert "root,root:60,0,root:7,0,MODEL" in result
        assert f"{SUBGRAPH_ID},{SUBGRAPH_ID}:45,0,{SUBGRAPH_ID}:19,0,CONDITIONING" in result
